...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $result = execute_query($conn, "SELECT id, password, login_attempts, lock_until FROM users WHERE username = ?", [1 => $username]);
    ...
    if ($user) {
        if ($user['lock_until'] && $user['lock_until'] > $now) {
            ...
            if (password_verify($password, $user['password'])) {
                execute_query($conn, "UPDATE users SET login_attempts = 0, lock_until = NULL WHERE id = ?", [1 => $user['id']]);
                ...
                if ($login_attempts >= 5) {
                    ...
                    execute_query($conn, "UPDATE users SET login_attempts = ?, lock_until = ? WHERE id = ?", 
                        [1 => $login_attempts, 2 => $lock_time, 3 => $user['id']]);
                    log_system_event($conn, $username, 'ACCOUNT_TRIGGERED_LOCKOUT');
                } else {
                    execute_query($conn, "UPDATE users SET login_attempts = ? WHERE id = ?", 
                        [1 => $login_attempts, 2 => $user['id']]);
